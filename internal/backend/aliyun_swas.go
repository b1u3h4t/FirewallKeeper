...
func NewAliyunSWAS(t config.Target, _ *config.Config) (*AliyunSWAS, error) {
	return &AliyunSWAS{
		name: t.Name,
...
func (b *AliyunSWAS) Name() string { return b.name }
...
func (b *AliyunSWAS) createRule(cfg *config.Config, proto, cidr, port string) error {
	...
	if err != nil {
		if isAliyunDuplicate(err) {
			log.Printf("[%s] 规则已存在，跳过: %s %s %s", b.name, cidr, proto, port)
			return nil
		}
		return fmt.Errorf("CreateFirewallRules: %w", err)
	}
	log.Printf("[%s] 已添加阿里云 SWAS 防火墙规则: %s %s %s", b.name, cidr, proto, port)
	return nil
}
...
func (b *AliyunSWAS) deleteRule(proto, cidr, port string) error {
	...
	log.Printf("[%s] 已删除旧阿里云 SWAS 防火墙规则: %s %s %s", b.name, cidr, proto, port)
	return nil
}
